...
public class BedrockingRecipeCategory implements IRecipeCategory<FakeInOutRecipe> {
    public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(LMod.MOD_ID, "bedrocking");
    ...
    public static final RecipeType<FakeInOutRecipe> BEDROCKING_RECIPE_TYPE =
            new RecipeType<>(UID, FakeInOutRecipe.class);

    private final IDrawable background;
    private final IDrawable icon;

    public BedrockingRecipeCategory(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 0, 0, 176, 91);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Blocks.BEDROCK));
    }

    @Override
    public RecipeType<FakeInOutRecipe> getRecipeType() {
        return BEDROCKING_RECIPE_TYPE;
    }

    @Override
    public Component getTitle() {
        return Component.literal("Bedrocking");
    }
